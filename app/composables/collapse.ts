...
export function useCollapse() {
  ...
  const menuItems = computed(() => {
    if (
      ...
      || app.roppa?.collapse?.navigation?.items?.length === 0
    ) {
      return []
    }
    return app.roppa?.collapse?.navigation?.items?.map(
      navigation =>
  ...
  const header = computed(() => {
    return app.roppa?.collapse?.navigation?.header
  })

  const footer = computed(() => {
    return app.roppa?.collapse?.navigation?.footer
  })
